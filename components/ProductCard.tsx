...
export const ProductCard: React.FC<ProductCardProps> = ({ product, isSellerView = false, onEdit, onQuickView, onUpdate }) => {
    ...
    const { addToCart, translations, comparisonList, toggleCompare, wishlist, toggleWishlist, currentBuyer } = context;
    ...
    return (
        <div onClick={() => onQuickView?.(product)} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden group flex flex-col relative cursor-pointer">
            <div className="absolute top-2 right-2 z-10 flex flex-col gap-2">
                ...
                {currentBuyer && !isSellerView && (
                    <button onClick={handleWishlistClick} title={isInWishlist ? "Remove from wishlist" : "Add to wishlist"} className={`p-2 rounded-full transition-all duration-200 ease-in-out ${isInWishlist ? 'text-red-500 bg-red-100 dark:bg-red-900/50' : 'bg-white/70 text-gray-700 hover:bg-white dark:bg-gray-900/50 dark:text-gray-200 dark:hover:bg-gray-900'}`}>
                 ...
                 {!isSellerView && (
                     <div className="absolute inset-0 bg-black/40 flex items-center justify-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        ...
                        <button 
                            ...
                            className="bg-secondary text-white p-3 rounded-full hover:bg-green-700 transition-colors disabled:bg-gray-500 disabled:cursor-not-allowed"
                        >
            ...
            <div className="p-2 md:p-3 flex flex-col flex-grow">
                <h3 className="text-sm md:text-base font-semibold text-gray-800 dark:text-gray-100 h-10 md:h-12">{product.name}</h3>
                ...
                <div className="flex justify-between items-center mt-auto pt-2">
                    <div>
                        {onSale ? (
                            <>
                                <span className="text-base md:text-lg font-bold text-red-600">${formattedPrice}</span>
                                <span className="text-xs text-gray-400 line-through ml-1.5">${formattedOriginalPrice}</span>
                            </>
                        ) : (
                           <span className="text-base md:text-lg font-bold text-primary dark:text-blue-400">${formattedPrice}</span>
                        )}
                    </div>
                </div>
            </div>
